...
if( isset($_POST['email']) && isset($_POST['password']) ) {
	...
	$login_teacher = auth::login_teacher($email, $password);

	if( $login_teacher ) {
		auth::setSessionId($login_teacher);
		header('location: schedules.php');
	}
	else {
		$error = 'Error: Invalid login combination.';
	}
}
?>
<!DOCTYPE html>
					...
					<div class="col-xs-12 col-sm-12 col-md-5 col-lg-4">
						<div class="well no-padding">
							<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post" id="login-form" class="smart-form client-form">
								...
								<?php if( isset($error) ) {
									printf('<p class="alert alert-danger">%s</p>', $error);
								}?>
								<footer>
